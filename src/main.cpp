...
uint256 hashGenesisBlock("0x12d1fff453711bff20f88f5f3834923185abb676b228587f6e0dfce56871a77f");
...
const string strMessageMagic = "ArkVentureCoin Signed Message:\n";
...
{
    // ArkVentureCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // ArkVentureCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "avc genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x73baf91da6e19590b3762689af3f125bcdf6f6d9b9f52e8c06bf5749cefd7343"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe2, 0xcc, 0xa1, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArkVentureCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArkVentureCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArkVentureCoinMiner(CWallet *pwallet)
{
    printf("ArkVentureCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ArkVentureCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ArkVentureCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArkVentureCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArkVentureCoinMiner, pwallet));
}
